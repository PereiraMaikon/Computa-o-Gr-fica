...
KEY_RIGHT = 39;  // Para Direita
    if (e.keyCode == KEY_UP && cube.position.y <=29){ //y - para cima 
        cube.position.y+=velocityY;
}
    if (e.keyCode == KEY_DOWN && cube.position.y >=-29){
        cube.position.y-=velocityY;
}
    if (e.keyCode == KEY_RIGHT && cube.position.x <= 70){ //x - para baixo
        cube.position.x +=velocity;
}
    if (e.keyCode == KEY_LEFT && cube.position.x >= -70){
        cube.position.x-=velocity;
}
    };
